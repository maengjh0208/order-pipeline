...
1. **1차 (walking skeleton)**: 주문 생성 → SSE로 상태 텍스트가 실시간으로 바뀌는 것까지만 — 재시도/DLQ/운영 대시보드/스타일링 없이 최소 형태로 브라우저에서 눈으로 확인
2. **2차**: 상태 머신 시각화(`OrderTimeline`), 주문 목록/생성 페이지 정식 구현
